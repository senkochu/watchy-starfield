...
typedef struct hourlyWeather {
    String date;      // ISO 8601 formatted date and time
    float rain;       // Precipitation as a fraction (0.0 to 1.0)
    int8_t temp;      // Temperature in degrees (assume Celsius)
} hourlyWeather;

typedef struct dailyWeather {
    hourlyWeather hourly[24]; // Array for up to 24 hourly weather entries
...
private:
  ...
  dailyWeather _getPrecipitation(String url);                                                        
};
