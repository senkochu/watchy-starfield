...
RTC_DATA_ATTR dailyWeather currentPrecip;
...
dailyWeather Watchy::getPrecipitation() {
  return _getWeatherData(settings.precipitationURL);
}

dailyWeather Watchy::_getPrecipitation(String url) { 
  if (connectWiFi()) {
    HTTPClient http; // Use Weather API for live data if WiFi is connected
    http.setConnectTimeout(3000); // 3 second max timeout
    String precipQueryURL = url;
    http.begin(precipQueryURL.c_str());
    int httpResponseCode = http.GET();
    if (httpResponseCode == 200) {
      String payload = http.getString(); // Get the response payload
      JSONVar responseArray = JSON.parse(payload); // Parse JSON
            
      if (JSON.typeof(responseArray) == JSONArray) { // Ensure it’s an array
        uint8_t entryCount = responseArray.length(); // Get array size
        currentPrecip.count = (entryCount > 24) ? 24 : entryCount; // Max 24 entries
              
        for (uint8_t i = 0; i < currentPrecip.count; i++) {
          currentPrecip.hourly[i].date = 
            (const char*)responseArray[i]["date"]; // Extract date
          currentPrecip.hourly[i].rain = 
            float(responseArray[i]["rain"]);      // Extract rain (0.0-1.0)
          currentPrecip.hourly[i].temp = 
            int(responseArray[i]["temp"]);       // Extract temperature
        }  
        } 
        // else { 
